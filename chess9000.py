    ...
    def draw_pieces(self):
        for square in chess.SQUARES:
            ...
            if piece:
                ...
                self.canvas.create_text(x, y, text=label, font=("Arial", 32))
    ...
    def on_click(self, event):
        ...
        if self.selected_square is None:
            ...
            if piece and piece.color == chess.WHITE:
                ...
                self.status.config(text=f"Selected {chess.square_name(square)}. Enter destination.")
        else:
            dest = simpledialog.askstring("Move", f"Move from {chess.square_name(self.selected_square)} to:")
            if dest:
                try:
                    move = chess.Move(self.selected_square, chess.parse_square(dest.strip()))
                    if move in self.board.legal_moves:
                        self.board.push(move)
                        self.selected_square = None
                        self.draw_board()
                        self.status.config(text="Enter Black's move (your friend's move)")
                        self.root.after(100, self.get_opponent_move)
                    else:
                        self.status.config(text="Illegal move. Try again.")
                        self.selected_square = None
                except:
                    self.status.config(text="Invalid square entered. Try again.")
                    self.selected_square = None

    def get_opponent_move(self):
        move_str = simpledialog.askstring("Opponent Move", "Enter your friend's move (e.g. e7e5):")
        try:
            ...
            if move in self.board.legal_moves:
                ...
                messagebox.showerror("Invalid", "That move is not legal.")
                self.get_opponent_move()
        except:
            messagebox.showerror("Invalid", "Bad format. Try again.")
            self.get_opponent_move()
