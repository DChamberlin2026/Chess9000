            ...
            if "pv" in info:
                ...
                if plan:
                    ...
                    try:
                        ...
                        move_str = str(plan[0])  # e2e4
                        self.status.config(text=f"Next move: {move_str[:2]} to {move_str[2:]} | Plan: " + " → ".join(line))
                    except:
                        self.status.config(text="AI suggested invalid move. Plan not shown.")
                else:
                    self.status.config(text="No clear winning plan.")
            else:
                self.status.config(text="Checkmate may no longer be possible.")
        except:
            self.status.config(text="Error analyzing position.")
